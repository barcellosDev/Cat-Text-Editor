...
export class TextEditor {
    ...
    getMaxRenderedBufferRow() {
        let hasChildren = this.UI.editorElement.children.length > 0

        if (!hasChildren)
            return

        let max = this.UI.editorElement.querySelector('.line:last-child').getAttribute('buffer-row')

        for (let index = 0; index < this.UI.editorElement.children.length; index++) {
            const bufferRow = Number(this.UI.editorElement.children[index].getAttribute('buffer-row'))
    ...
    getMinRenderedBufferRow() {
        ...
        let min = this.UI.editorElement.children[0].getAttribute('buffer-row')
